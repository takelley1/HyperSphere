...
func TestDefaultCatalogProvidesExpandedBrowsingDataset(t *testing.T) {
	...
	if len(catalog.Datacenters) < 3 {
		t.Fatalf(
			"expected at least 3 datacenter rows for browsing, got %d",
	...
	if len(catalog.Networks) < 6 {
		t.Fatalf(
			"expected at least 6 network rows for browsing, got %d",
			len(catalog.Networks),
		)
	}
	if len(catalog.Templates) < 6 {
		t.Fatalf(
			"expected at least 6 template rows for browsing, got %d",
	...
	if len(catalog.Tasks) < 6 {
		t.Fatalf("expected at least 6 task rows for browsing, got %d", len(catalog.Tasks))
	}
	...
	if len(catalog.Folders) < 6 {
		t.Fatalf("expected at least 6 folder rows for browsing, got %d", len(catalog.Folders))
	}
	if len(catalog.Tags) < 6 {
		t.Fatalf("expected at least 6 tag rows for browsing, got %d", len(catalog.Tags))
	}
